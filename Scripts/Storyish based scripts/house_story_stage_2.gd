	...
	if GameManager.StoryStage == 2:
		...
		if StoryStage2MainCutscene:
			if Input.is_action_just_pressed("LMB") && !sit_down.is_hovered() && !GameManager.TextSlowShowing && dialWait:
				...
				match dialogue:
					2:
						GameManager.SetDialog("Pero handa na naman ang tanghalian, sumabay ka na lang sa amin.", true, 1, GameManager._1_sister_calm_nohat, 2) # sis 1
					3:
						...
						GameManager.SetDialog("Anak, salamat at dumating ka para matulungan kami, pasensya na lang sa abala.", true, 1, GameManager._2_dad_confused_nohat, 3) # dad 2
						dialWait = true
					4:
					...
					6:
						GameManager.SetDialog("Sige anak. Para magsimula tayo, ngayong hapon na ito, tuturuan ko ikaw sa palay namin, ulit.", true, 1, GameManager._1_dad_calm_nohat, 3) # dad 1
					7:
						dialWait = false
					...
					9:
						...
						GameManager.SetDialog("Eh paano ga, kalalaki mong tao ay ikaw pa ang maarte sa mga gawaing noong bata pa tayo.", true, 1, GameManager._6_sister_joyful_nohat, 2) # sis 6
					10:
						GameManager.SetDialog("O tama na kayong dalawa, dahil dyan kayo ang magtutulungan ngayon sa palay.", true, 1, GameManager._3_dad_angry_nohat, 3) # dad 3
					11:
						GameManager.SetDialog("Marunong ka na rin Ellia, ikaw na ang magturo sa kapatid mo dun sa pag-refine ng bigas.", true, 1, GameManager._3_dad_angry_nohat, 3) # dad 3
					12:
...
func _on_sit_down_button_down():
	...
	await get_tree().create_timer(1).timeout
	dialogue = 1
	GameManager.SetDialog("Tanghali ka na, naubosan ka ng almusal kuya.", true, 1, GameManager._1_sister_calm_nohat, 2) # sister 1
